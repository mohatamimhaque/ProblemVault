...
bool isSquare(double side, double diagonal) {
    double expectedDiagonal = side * sqrt(2);
    return fabs(diagonal - expectedDiagonal) < 1e-9;
}
void solve(){
    ...
    int s1 = distance(0,u,r,0);
    int s2 = distance(r,0,0,-d);
    int s3 = distance(0,-d,-l,0);
    int s4 = distance(-l,0,0,u);

    bool side = (s1 == s2) && (s2 == s3) && (s3 == s4) && (s4 == s1);

    int d1 = distance(0,u,0,-d);
    int d2 = distance(-l,0,r,0);

    bool dia = d1 ==d2;
    if(side && isSquare(sqrt(s1),sqrt(d1)) && dia){
        cout<<"Yes\n";
    }else{
        cout<<"No\n";
    }

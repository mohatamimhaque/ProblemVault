...
int power(int x, int y, int p = mod) { int res = 1; x %= p; while (y > 0) { if (y & 1) res = (res * x) % p; y >>= 1; x = (x * x) % p; } return res; }
...
void solve(int t) {
    int n;cin>>n;
    vector<int>boy(n);cin>>boy;
    sort(boy.begin(),boy.end());

    int m;cin>>m;
    vector<int>girl(m);cin>>girl;
